...
int main(int args, char **argv) {
	...
	ans = fopen(argv[1], "r");			//開啟required.txt
	if (ans == NULL) {
		...
		mine = fopen(argv[2], "r");			//開啟derived.txt
		if (mine == NULL) {
			...
			for (int i = 0; i < N; i++) {			//跑 N 次
				fscanf(ans, "%[^\n]", STRans);			//讀取字串至換行符號
				fseek(ans, 1, SEEK_CUR);			//將檔案內指標切換到下一行
				fscanf(mine, "%[^\n]", STRmine);			//讀取字串至換行符號
				fseek(mine, 1, SEEK_CUR);			//將檔案內指標切換到下一行
